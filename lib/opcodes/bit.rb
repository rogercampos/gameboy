Instruction.define do
  ...
  (0..7).each.with_index do |n_bit, bit_index|
    ...
    opcode(0xcb40 + bit_index * 8 + 6, 16, 1) { Flags.n = 0; Flags.h = 1; Flags.z = 1 if MMU.read(Registers.hl)[n_bit] == 0 }
  ...
  (0..7).each.with_index do |n_bit, bit_index|
    ...
    opcode(0xcb80 + bit_index * 8 + 6, 16, 1) { foo = MMU.read(Registers.hl); foo &= (0b1111_1111 ^ (1 << n_bit)); MMU.write(Registers.hl, foo) }
  ...
  (0..7).each.with_index do |n_bit, bit_index|
    ...
    opcode(0xcbc0 + bit_index * 8 + 6, 16, 1) { foo = MMU.read(Registers.hl); foo |= 1 << n_bit; MMU.write(Registers.hl, foo) }
